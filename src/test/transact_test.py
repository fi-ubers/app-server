...
from src.main.resources import UserTransactions
...
class TestUserTransactions(object):
	...
	def test_getall_user_transact_unauthorized(self, validateTokenMock):
		...
		response = self.app.get(V1_URL + "/users/1/transactions", headers={"UserToken" : "A fake token"})
	...
	def test_getall_user_transact_authorized(self, validateTokenMock, FakeGet):
		...
		assert(response_parsed["transactions"] == expected["transactions"])
